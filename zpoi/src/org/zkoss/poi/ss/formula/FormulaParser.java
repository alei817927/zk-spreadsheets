...
*/
	...
	private ParseNode parseRangeable() {
		...
		if (part1 == null) {
			...
			if (look == '[') { // could be a no name Table reference
				return parseColumnName(null, _sheetIndex);
			} else if (Character.isLetter(look) || look != '_' || look != '\\') {
				String tableName = parseName(); // could be a table name
				if (tableName != null && look == '[') {
					return parseColumnName(tableName, _sheetIndex);
				}
			}
	...
	//ZSS-796
	private ParseNode parseColumnName(String tableName, int sheetIndex) {
		SkipWhite();
		GetChar();
		// from now on we can only be dealing with non-quoted identifiers
		// which will either be named ranges or functions
		...
		while (isValidDefinedNameChar(look) || look == ' ') {
			sb.append(look);
			GetChar();
		}
		SkipWhite();
		...
		GetChar();
		String columnName = sb.toString().trim();
		return createTableRef(tableName, columnName, sheetIndex); 
	}
	
	//ZSS-796
	private ParseNode createTableRef(String tableName, String columnName, int sheetIndex) {
		//TODO: rowIdx, colIdx, function name!
		TableNamePtg namePtg = (TableNamePtg) _book.getTableName(tableName, columnName, sheetIndex, /*rowIdx*/ -1, /*colIdx*/-1).createPtg();
		return getFunction("_zss_TableRef", namePtg, new ParseNode[0]);
	}
